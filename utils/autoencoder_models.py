...
class AutoEncoder_vgg16(nn.Module):
    def __init__(self, dimension):
        ...
        dimension = dimension//4
        self.encoder = nn.Sequential(
            nn.Conv2d(512, 1024, 5, stride=2, padding=0), ###512*26*26 -> 11*11
            nn.BatchNorm2d(1024),
            nn.PReLU(),
             
            nn.Conv2d(1024, 640, 3, stride=1, padding=0), ###11*11 -> 9*9
            nn.BatchNorm2d(640),
            nn.PReLU(),

            nn.Conv2d(640, 256, 3, stride=2, padding=0), ###9*9 -> 4*4
            nn.BatchNorm2d(256),
            nn.PReLU(),
            
            nn.Conv2d(256, dimension, 3, stride=1, padding=0), ###4*4 -> 2*2
            nn.BatchNorm2d(dimension),
            nn.PReLU(),
        )

        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(dimension, 256, 3, stride=1, padding=0),  #dimension*2*2 -> 4*4
            nn.BatchNorm2d(256),
            nn.PReLU(),

            nn.ConvTranspose2d(256, 640, 3, stride=2, padding=0),  #4*4 -> 9*9 
            nn.BatchNorm2d(640),
            nn.PReLU(),

            nn.ConvTranspose2d(640, 1024, 4, stride=1, padding=0),  #9*9 ->12*12
            nn.BatchNorm2d(1024),
            nn.PReLU(),

            nn.ConvTranspose2d(1024, 512, 4, stride=2, padding=0),  #12*12 -> 26*26
            nn.BatchNorm2d(512),
            nn.PReLU(),
        )
